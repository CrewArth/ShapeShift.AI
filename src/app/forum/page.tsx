...
export default function Forum() {
  ...
  const handleDownload = async (modelUrl: string) => {
    if (!isSignedIn) {
      window.location.href = 'https://humorous-oarfish-98.accounts.dev/sign-in';
      return;
    }
          ...
          <div className="modal-overlay" onClick={() => setSelectedModel(null)}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-body">
                ...
                <button 
                  className={`download-button ${!isSignedIn ? 'opacity-50' : ''}`}
                  onClick={() => handleDownload(selectedModel.modelUrl)}
                >
                  <Download size={20} />
                  {isSignedIn ? 'Download Model' : 'Sign in to Download'}
                </button>
              </div>
            </div>
          </div>
